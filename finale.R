...
c%>%
  select(z1,z2,z3,z4,z5,z6,z7,z8,z9) %>%
...
d <- subset(data,data$Dispo_conso_ins_ingredients=="Pas du tout" | data$Dispo_conso_ins_entiers=="Pas du tout" | data$Dispo_conso_ins_ingredients=="Plutôt non" | data$Dispo_conso_ins_entiers=="Plutôt non")
d%>%
...
tbl <- data%>%
  select(Dispo_conso_ins_entiers,v1,v3,v4)%>%
  tbl_summary(label = list(Dispo_conso_ins_entiers~"Disposition à consommer des produits contenant des insectes comestibles préparés entiers",
                           v1~"Restaurants / Traiteurs",
                           v3~"Hard-discounts (ex. Lidl, Leader price, Aldi, Netto.)",
                           v4~"Commerces de proximité"
  ),
  include = c(Dispo_conso_ins_entiers,v1,v3,v4),
  by=Dispo_conso_ins_entiers,
  missing = "no",
  statistic = list(all_continuous()~"{mean}({sd})")
  ,percent="row")
...
tbl <- data%>%
  select(Dispo_conso_ins_ingredients,Alim_valeurs,Alim_croyances,Alim_ouverture)%>%
  tbl_summary(label = list(Dispo_conso_ins_ingredients~"Disposition à consommer des produits contenant des insectes comme ingrédients",
                           ...
                           Alim_ouverture~"Je suis ouvert à de nouvelles habitudes alimentaires"
  ),
  include = c(Dispo_conso_ins_ingredients,Alim_valeurs,Alim_croyances,Alim_ouverture),
  by=Dispo_conso_ins_ingredients,
  missing = "no",
  statistic = list(all_continuous()~"{mean}({sd})")
  ,percent="row")
...
data$Alim_ouverture <- data$Alim_ouverture%>%
  fct_relevel("Pas du tout d'accord","Plutôt pas d'accord","Plutôt d'accord","Tout à fait d'accord")
tbl <- data%>%
  select(Conso_ins,Alim_ouverture)%>%
  tbl_summary(label = list(Conso_ins~"Consommation d'insectes",
                           Alim_ouverture~"Je suis ouvert à de nouvelles habitudes alimentaires"
                        
  ),
  include = c(Conso_ins,Alim_ouverture),
  by=Conso_ins,
  missing = "no",
  statistic = list(all_continuous()~"{mean}({sd})")
  ,percent="row")
...
tbl%>%
  modify_spanning_header(all_stat_cols()~"**Consommation d'insectes**")%>%add_p()%>%modify_header(update=list(label~"**Variables**"))%>%add_significance_stars(thresholds = c( 0.01, 0.05,0.1))
   ...
   # [33] "Avis_ins_secu_alim" 
tbl <- data%>%
  select(Conso_ins,Avis_ins_secu_alim)%>%
  tbl_summary(label = list(Conso_ins~"Consommation d'insectes",
                           Avis_ins_secu_alim~"La production de produits alimentaires à base d'insectes est une solution durable qui peut permettre de garantir la sécurité alimentaire mondiale"
  ),
  include = c(Conso_ins,Avis_ins_secu_alim),
  by=Conso_ins,
  missing = "no",
  statistic = list(all_continuous()~"{mean}({sd})")
  ,percent="row")
tbl%>%
  modify_spanning_header(all_stat_cols()~"**Consommation d'insectes**")%>%add_p()%>%modify_header(update=list(label~"**Variables**"))%>%add_significance_stars(thresholds = c( 0.01, 0.05,0.1))
   ...
   # [34] "Avis_ins_benefices_nutri"
tbl <- data%>%
  select(Conso_ins,Avis_ins_benefices_nutri)%>%
  tbl_summary(label = list(Conso_ins~"Consommation d'insectes",
                           Avis_ins_benefices_nutri~"Les produits alimentaires à base d'insectes présentent d'importants bénéfices nutritionnels"
  ),
  include = c(Conso_ins,Avis_ins_benefices_nutri),
  by=Conso_ins,
  missing = "no",
  statistic = list(all_continuous()~"{mean}({sd})")
  ,percent="row")
tbl%>%
  modify_spanning_header(all_stat_cols()~"**Consommation d'insectes**")%>%add_p()%>%modify_header(update=list(label~"**Variables**"))%>%add_significance_stars(thresholds = c( 0.01, 0.05,0.1))
   # [35] "Avis_ins_sante" 
tbl <- data%>%
  select(Conso_ins,Avis_ins_sante)%>%
  tbl_summary(label = list(Conso_ins~"Consommation d'insectes",
                           Avis_ins_sante~"Les produits alimentaires à base d'insectes sont bénéfiques pour la santé"
  ),
  include = c(Conso_ins,Avis_ins_sante),
  by=Conso_ins,
  missing = "no",
  statistic = list(all_continuous()~"{mean}({sd})")
  ,percent="row")
tbl%>%
  modify_spanning_header(all_stat_cols()~"**Consommation d'insectes**")%>%add_p()%>%modify_header(update=list(label~"**Variables**"))%>%add_significance_stars(thresholds = c( 0.01, 0.05,0.1))
   # [36] "Avis_ins_alternative"   
tbl <- data%>%
  select(Conso_ins,Avis_ins_alternative)%>%
  tbl_summary(label = list(Conso_ins~"Consommation d'insectes",
                           Avis_ins_alternative~"Les produits alimentaires à base d'insectes représentent une alternative satisfaisante aux protéines animales"
  ),
  include = c(Conso_ins,Avis_ins_alternative),
  by=Conso_ins,
  missing = "no",
  statistic = list(all_continuous()~"{mean}({sd})")
  ,percent="row")
...
#################
tbl <- data%>%
  select(Conso_ins,Avis_ins_impacts_env,Avis_ins_secu_alim,Avis_ins_benefices_nutri,Avis_ins_sante,Avis_ins_alternative)%>%
  tbl_summary(label = list(Conso_ins~"Consommation d'insectes"
  ),
  include = c(Conso_ins,Avis_ins_impacts_env,Avis_ins_secu_alim,Avis_ins_benefices_nutri,Avis_ins_sante,Avis_ins_alternative),
  by=Conso_ins,
  missing = "no",
  statistic = list(all_continuous()~"{mean}({sd})")
  ,percent="row")
...
data$Avis_ins_secu_alim <- data$Avis_ins_secu_alim%>%
  fct_relevel("Pas du tout d'accord","Plutôt pas d'accord","Plutôt d'accord","Tout à fait d'accord","Ne sais pas")
tbl <- data%>%
  select(Dispo_conso_ins_ingredients,Avis_ins_secu_alim)%>%
  tbl_summary(label = list(Dispo_conso_ins_ingredients~"Consommation d'insectes",
                           Avis_ins_secu_alim~"La production de produits alimentaires à base d'insectes est une solution durable qui peut permettre de garantir la sécurité alimentaire mondiale"
  ),
  include = c(Dispo_conso_ins_ingredients,Avis_ins_secu_alim),
  by=Dispo_conso_ins_ingredients,
  missing = "no",
  statistic = list(all_continuous()~"{mean}({sd})")
  ,percent="row")
tbl%>%
  modify_spanning_header(all_stat_cols()~"**Disposition à consommer des produits contenant d'insectes comme ingrédients**")%>%add_p()%>%modify_header(update=list(label~"**Variables**"))%>%add_significance_stars(thresholds = c( 0.01, 0.05,0.1))

data$Avis_ins_secu_alim <- ifelse(Avis_ins_secu_alim=="Pas du tout d'accord" | Avis_ins_secu_alim==" Plutôt pas d'accord","Pas d'accord",Avis_ins_secu_alim)
tbl <- data%>%
  select(Dispo_conso_ins_entiers,Avis_ins_secu_alim)%>%
  tbl_summary(label = list(Dispo_conso_ins_entiers~"Consommation d'insectes"
  ),
  include = c(Dispo_conso_ins_entiers,Avis_ins_secu_alim),
  by=Dispo_conso_ins_entiers,
  missing = "no",
  statistic = list(all_continuous()~"{mean}({sd})")
  ,percent="row")
tbl%>%
  modify_spanning_header(all_stat_cols()~"****")%>%add_p()%>%modify_header(update=list(label~"**Variables**"))%>%add_significance_stars(thresholds = c( 0.01, 0.05,0.1))
